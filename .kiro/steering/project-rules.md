# Project Rules

## Git Workflow

- You MUST commit after completing each discrete feature, fix, or refactor — do not batch unrelated changes.
- Use Conventional Commits format: `type(scope): description`
- If you realize uncommitted work exists from a previous change, commit it before starting new work.
- Pushing to remote is allowed. Use `git push` when the user asks.

## Code Style

- Write minimal code. Avoid verbose implementations.
- Only save files to the deal-finder project folder or `~/Documents/Obsidian Vault`.

## Session Start

- At the start of each session, run `./refresh.sh --all -c 10 -p 3` to ensure listings are up to date.
